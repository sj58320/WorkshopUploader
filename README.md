...
EXE 하나로 CS2 에셋을 멀티청크 VPK로 만들고 Steam 창작마당에 업로드하는 Windows 프로그램입니다. 개인 로컬 폴더만 사용하는 모드와 사용자가 지정한 GitHub 저장소를 함께 동기화하는 GitHub 모드를 지원합니다.

## 다운로드

[Releases](https://github.com/sj58320/WorkshopUploader/releases)에서 `WorkshopUploader-v0.4.1.exe`만 받아 실행하면 됩니다. Python, VPKEdit CLI, SteamworksPy와 필요한 DLL은 EXE 안에 포함되어 있습니다. 실제 에셋 파일은 포함되지 않으며 사용자가 폴더를 선택해야 합니다.
...
4. 로컬 모드에서는 `에셋 폴더`를 선택합니다. GitHub 모드에서는 `저장소`, `브랜치`, `에셋 경로`를 확인한 뒤 로그인과 최신화가 끝날 때까지 기다립니다. 기존 사용자는 기본값을 그대로 사용하면 됩니다.
5. `출력 폴더`를 선택합니다. 기본값을 그대로 사용해도 됩니다.
6. 바꿀 배너가 있으면 `미리보기 이미지`에서 PNG/JPG/JPEG/GIF 파일을 선택합니다. 비워 두면 현재 창작마당 이미지를 유지합니다.
7. 여러 줄로 `업데이트 내역`을 적습니다. 비워 두면 `Update asset`을 사용합니다.
8. 먼저 `VPK만 만들기`로 결과를 확인한 다음 `Steam 창작마당 업로드`를 누릅니다.
...
- 미리보기 이미지: Steam 창작마당 대표 이미지로 올릴 PNG/JPG/JPEG/GIF 파일

예전 Python 업로더는 `Workshop_Pack\in\rss_banner.jpg`를 고정해서 Steam에 전달했습니다. 현재 GUI에서는 이미지를 특정 폴더로 복사할 필요가 없습니다. `찾아보기`를 눌러 기존 `Workshop_Pack\in\rss_banner.png`를 직접 선택해도 되고 다른 위치의 이미지도 선택할 수 있습니다.

선택한 경로는 다음 실행에도 유지됩니다. 입력란을 비우면 다음 업로드에서 Steam의 기존 미리보기 이미지를 변경하지 않습니다.
...
GitHub 모드의 기본값은 다음과 같으며 화면에서 다른 저장소로 바꿀 수 있습니다.

```text
저장소: RevenantZE/RSS-ZE-ASSET
브랜치: main
...
저장소와 브랜치마다 `%LOCALAPPDATA%\WorkshopUploader\repos` 아래에 서로 다른 로컬 clone 폴더를 사용합니다. 같은 EXE를 사용하는 다른 사람도 자기 저장소를 입력할 수 있지만, WorkshopUploader GitHub App을 그 저장소에 설치하고 로그인한 계정에 push 권한을 부여해야 합니다. GitHub 권한과 Steam 창작마당 기여자 권한은 서로 별개입니다.

선택한 저장소와 브랜치를 처음 사용할 때는 한 번 내려받으므로 전체 에셋 용량만큼 시간이 걸립니다. 다음 실행부터는 `git fetch`와 fast-forward 방식으로 변경된 Git 객체만 내려받습니다. 원격 변경을 받는 동안 로컬 수정은 임시 보관했다가 다시 적용하며, 충돌이 나면 파일을 자동 삭제하거나 덮어쓰지 않고 작업을 잠급니다.
...
4. 화면에서 선택한 저장소와 브랜치에 push

Steam 업로드가 성공한 뒤 GitHub push만 실패하면 복구 정보가 `%LOCALAPPDATA%\WorkshopUploader\pending_upload.json`에 남습니다. 다음 실행의 `GitHub Push 재시도`는 Steam에 다시 올리지 않고 GitHub 단계만 이어서 처리합니다. 미완료 push가 있는 동안에는 소스 모드와 저장소 설정을 변경할 수 없습니다. 재시도는 업로드 당시 기록한 저장소·브랜치·에셋 경로만 사용합니다.
...
GitHub에 push하려면 WorkshopUploader GitHub App이 선택한 저장소에 설치되어 있어야 하고, 프로그램에서 로그인한 사용자도 해당 저장소에 push 권한을 가져야 합니다. Steam 창작마당 기여자 권한과 GitHub 저장소 권한은 서로 별개입니다.
...
릴리스에서 직접 사용하는 첨부 파일은 `WorkshopUploader-v0.4.1.exe` 하나입니다. GitHub가 모든 릴리스에 자동으로 붙이는 `Source code (zip)`과 `Source code (tar.gz)`는 저장소 전체의 스냅샷이라 삭제할 수 없습니다.
...
64비트 Python 3.10 이상이 필요합니다. GitHub App의 Device Flow를 켜고 Contents 읽기·쓰기 권한과 `Any account` 설치 대상을 설정한 뒤 client ID를 환경 변수에 넣어야 합니다. 사용자는 동기화할 각 저장소에 이 App을 설치해야 하며 client secret은 사용하지 않습니다.

```powershell
python -m pip install -r requirements-build.txt
$env:WORKSHOP_UPLOADER_GITHUB_CLIENT_ID = "GitHub App client ID"
powershell -ExecutionPolicy Bypass -File .\scripts\build_release.ps1 -Version v0.4.1
```

빌드 스크립트는 공식 MinGit 2.55.0.3 압축 파일을 내려받아 고정된 SHA-256을 확인하고 EXE 안에 포함합니다. 결과물은 `dist/WorkshopUploader-v0.4.1.exe` 하나이며 실제 에셋이나 생성된 VPK는 포함하지 않습니다.
